        ...
        for f in range(F):
            for i in range(Hout):
                for j in range(Wout):
                    hh = i * stride
                    ww = j * stride
                    dx_padded[n, :, hh:hh + HH, ww:ww + WW] += dout[n, f, i, j] * w[f]
                    dw[f] += dout[n, f, i, j] * x_padded[n, :, hh:hh + HH, ww:ww + WW]
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    hh = i * stride
                    ww = j * stride
                    out[n, c, i, j] = np.max(x[n, c, hh:hh + pool_height, ww:ww + pool_width])
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    hh = i * stride
                    ww = j * stride
                    sub_matrix = x[n, c, hh:hh+pool_height, ww:ww+pool_width]
                    # only agrmax has contribution, while the rest have zero gradient
                    coordinate = np.unravel_index(sub_matrix.argmax(), sub_matrix.shape)
                    dx[n, c, hh + coordinate[0], ww + coordinate[1]] = dout[n, c, i, j]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    x_reshape = x.swapaxes(1, 3).reshape(-1, C)  # (N, C, H, W) -> (N*H*W, C)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    N, C, H, W = dout.shape
    dout_reshape = dout.swapaxes(1, 3).reshape(-1, C)  # (N, C, H, W) -> (N*H*W, C)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dx_mean = np.sum(dx_hat_T * (-1 / np.sqrt(x_var + eps)), axis=0) + np.sum(-2 * dx_var * (x - x_mean), axis=0) / Nprime

    dx_group_T = dx_hat_T / np.sqrt(x_var + eps) + dx_var * 2 * (x - x_mean) / Nprime + dx_mean / Nprime
