...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # output layer
        dx, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = affine_backward(dscores, cache)
        ...
        for i in reversed(range(1, self.num_layers)):
            if self.normalization == None:
                ...
                dx, grads['W' + str(i)], grads['b' + str(i)], grads['gamma' + str(i)], grads['beta' + str(i)] =\
                    affine_norm_relu_backward(dx, caches[i-1])
...
def affine_norm_relu_forward(x, w, b, gamma, beta, bn_param):
    """
    Convenience layer that perorms an affine transform followed by a normalization layer, 
    and then followed by a ReLU
    ...
    if len(bn_param) == 0:  # layernorm
        bn, bn_cache = layernorm_forward(a, gamma, beta, bn_param)
    else:  # batchnorm
        bn, bn_cache = batchnorm_forward(a, gamma, beta, bn_param)
    out, relu_cache = relu_forward(bn)
    cache = (fc_cache, bn_cache, relu_cache, bn_param)
    return out, cache
...
def affine_norm_relu_backward(dout, cache):
    ...
    """
    fc_cache, bn_cache, relu_cache, bn_param = cache
